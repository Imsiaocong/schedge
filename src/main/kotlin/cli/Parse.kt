...
internal class Parse : CliktCommand(name = "parse") {
    ...
    private class School : CliktCommand(name = "school") {
        private val logger = KotlinLogging.logger("parse.school")
        private val prettyPrint by option("--pretty").flag(default = false)
        private val inputFile by argument(help = "The file to read from. If not provided, reads from stdin.")
        private val outputFile by option(
                "--output-file",
        ...
        override fun run() {
            val input = inputFile.readFromFileOrStdin()
            val schools = ParseSchoolSubjects.parseSchool(input)
            val subjects = ParseSchoolSubjects.parseSubject(input)
            outputFile.writeToFileOrStdout(JsonMapper.toJson(schools, prettyPrint))
            outputFile.writeToFileOrStdout(JsonMapper.toJson(subjects, prettyPrint))
        }
    }
}
